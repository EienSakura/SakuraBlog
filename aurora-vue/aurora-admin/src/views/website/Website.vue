        ...
        <el-form label-width="100px" :model="websiteConfigForm" label-position="left">
          ...
          <el-form-item label="首页背景图">
            <el-upload
              ...
              :show-file-list="false"
              :before-upload="beforeUpload"
              :on-success="handleHeroBackgroundSuccess">
              <img v-if="websiteConfigForm.heroBackground" :src="websiteConfigForm.heroBackground" class="avatar" />
              <i v-else class="el-icon-plus avatar-uploader-icon" />
            </el-upload>
          </el-form-item>
          ...
          <el-form-item label="一言来源">
            <el-radio-group v-model="websiteConfigForm.hitokotoSource">
              <el-radio label="hitokoto">Hitokoto</el-radio>
              <el-radio label="custom">自定义</el-radio>
              <el-radio label="builtin">默认内置</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="自定义一言" v-if="websiteConfigForm.hitokotoSource === 'custom'">
            <el-input
              v-model="websiteConfigForm.hitokotoCustomText"
              placeholder="每行一条，将按顺序循环显示"
              style="width: 400px"
              type="textarea"
              :rows="4" />
          </el-form-item>
  ...
  methods: {
    getWebsiteConfig() {
      this.axios.get('/api/admin/website/config').then(({ data }) => {
        ...
        if (!this.websiteConfigForm.hitokotoSource) {
          this.websiteConfigForm.hitokotoSource = 'hitokoto'
        }
        if (!this.websiteConfigForm.hitokotoCustomText) {
          this.websiteConfigForm.hitokotoCustomText = ''
        }
      })
    },
    ...
    handleHeroBackgroundSuccess(response) {
      this.websiteConfigForm.heroBackground = response.data
    },
