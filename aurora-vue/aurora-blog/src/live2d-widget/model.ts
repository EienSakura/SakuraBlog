...
import type { AppDelegate as Cubism5Model } from './cubism5/index';
...
class ModelManager {
  ...
  resetCanvas() {
    ...
    if (waifuCanvas) {
      waifuCanvas.innerHTML = '<canvas id="live2d" width="800" height="800"></canvas>';
    }
  }
  ...
  async loadLive2D(modelSettingPath: string, modelSetting: object) {
    ...
    try {
      ...
      if (version === 2) {
        ...
        const { AppDelegate: Cubism5Model } = await import('./cubism5/index');
        ...
        if (this.currentModelVersion === 2 || !this.cubism5model?.subdelegates?.at(0)) {
          ...
          await this.cubism5model?.changeModel(modelSettingPath);
        }
      }
