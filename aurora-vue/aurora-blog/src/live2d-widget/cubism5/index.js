...
// 暂时禁用 cubism5 模块，因为缺少依赖文件
import logger from '../logger';

class Cubism5Model {
  constructor() {
    logger.warn('Cubism5 model is not available due to missing dependencies');
  }
  
  async init(canvasId, modelSettingPath, modelSetting) {
    logger.warn('Cubism5 model initialization failed: dependencies missing');
  }
  
  destroy() {
    logger.warn('Cubism5 model destroy called');
  }
}
