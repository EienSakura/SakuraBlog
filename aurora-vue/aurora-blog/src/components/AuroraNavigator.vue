    ...
    const scrollHandler = () => {
      ...
      setTimeout(() => {
        ...
        if (isHome.value) {
          ...
          scrollTop = Math.max(0, scrollTop - winHeight)
          docHeight = Math.max(winHeight, docHeight - winHeight)
        }

        const calculated = (scrollTop / (docHeight - winHeight)) * 100
        progress.value = Number(Math.min(100, Math.max(0, calculated)).toFixed(0))
      }, 0)
    }
